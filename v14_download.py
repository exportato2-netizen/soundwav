...
def convert(source:Path,info:dict[str,Any],bits:int,keep:bool=False,*,overwrite:bool=False)->Path:
    ...
    if not clean_wav(p):
        p.unlink(missing_ok=True); raise RuntimeError('El WAV final no superó la verificación fmt/data')
    return p
    ...
    try:
        ...
        with app.STATE.lock:
            if app.STATE.cancel_requested: app.STATE.status='cancelled'; app.STATE.message='Descarga cancelada'
                ...
                app.STATE.status='completed'; app.STATE.percent=100; app.STATE.message=f"Finalizado: {app.STATE.completed} nuevas · {app.STATE.skipped} ya existentes" if (app.STATE.completed or app.STATE.skipped) else 'No había pistas nuevas para descargar'
...
def patch_html(app:Any)->None:
    ...
    h=h.replace("const base=s.total?`${s.completed}/${s.total} pistas`:`${s.completed} pistas`;$('count').textContent=s.failed?`${base} · ${s.failed} error(es)`:base;","const base=s.total?`${s.completed}/${s.total}`:`${s.completed}`;const skip=s.skipped?` · ${s.skipped} existentes`:'';$('count').textContent=`Pistas: ${base}${skip}`;")
...
def apply(app:Any)->None:
    ...
    def post(self):
        ...
        except Exception as e:self.send_json({'error':f'Solicitud inválida: {e}'},app.HTTPStatus.BAD_REQUEST)
